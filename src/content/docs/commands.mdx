...
roborev analyze --mode security ./...              # Security-focused analysis
...
| `-m, --model <model>` | Model to use |
| `--reasoning <level>` | Set reasoning depth |
| `--mode <mode>` | Analysis mode (`security`) to apply a focused system prompt on top of the analyze type |
...
| Flag | Description |
|------|-------------|
| `--json` | Emit daemon and queue status as JSON. Includes the active daemon `endpoint` (TCP address or Unix socket path) alongside queue counters and version fields |
